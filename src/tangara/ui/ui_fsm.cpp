...
std::unique_ptr<drivers::Sh1106> UiState::sDisplay;
...
lua::Property UiState::sDisplayBrightness{
    0, [](const lua::LuaValue& val) {
      ...
      sDisplay->SetContrast(*brightness);
...
void UiState::react(const internal::InitDisplay& ev) {
  ...
  (void)init_data;  // legacy display init kept for reference only.
  sDisplay.reset(drivers::Sh1106::Create());
