...
auto Booting::entry() -> void {
  ...
  // Real button I/O for the new board: PCA8575 on I2C1.
  sServices->pca8575(std::unique_ptr<drivers::Pca8575>(
      drivers::Pca8575::Create()));

  // FM tuner: power up the FM rail (TPS25961 EN/UVLO via PCA8575:P00),
  // then probe the RDA5807FP on I2C1.  Ignore failure: the radio is an
  // optional peripheral.
  if (sServices->pca8575()) {
    sServices->pca8575()->SetFmRail(true);
    sServices->pca8575()->Flush();
    vTaskDelay(pdMS_TO_TICKS(50));
  }
  sServices->radio(std::unique_ptr<drivers::Rda5807>(
      drivers::Rda5807::Create(drivers::i2c1_handle())));
